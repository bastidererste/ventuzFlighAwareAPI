...
public class Script : ScriptBase, System.IDisposable
{
	...
	public void enroute_complete(object sender, EnrouteCompletedEventArgs e){
		...
		{	  
			...
			foreach(EnrouteFlightStruct efs in e.Result.enroute){
				Ventuz.Kernel.VLog.Info("info", efs.origin);
				tempArray[i] = efs.origin;
				i++;
			}
		...
		catch (Exception ex)
		{
			...
			Ventuz.Kernel.VLog.Info("error", ex.ToString());
			
		}
	
	}
	
	// This Method is called if the function/method getData is invoked by the user or a bound event.
	// Return true, if this component has to be revalidated!
	public bool OngetData(int arg)
	{
		df.EnrouteAsync("KLAX", 10, "", 0);
		return false;
	}
	...
	public bool Onconnect(int arg)
	{
		...
		try 
		{
			...
			df.EnrouteCompleted += new EnrouteCompletedEventHandler(enroute_complete);
